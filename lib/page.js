...
runbook.Page = class Page {
  ...
  /*
    Form methods
  */
  fillAndSubmitForm(action, userData, score_threshold=.5) {
    ...
    matches.forEach(match => {
      if (match.value == null) {
        if (match.dtype)
          all_promises.push(userData.getUserValueForDataType(match).then(value => {match.value = value; return match;}));
        else
          all_promises.push(userData.getUserValueForName(match.name).then(value => {match.value = value; return match;}));
      } else {
    ...
    let p = Promise.all(all_promises).then(matches => {
      matches.forEach(m => runbook.Automator.fillFormInput(m));
      runbook.Automator.submitForm(form);
    });
  ...
  */
  goto(action) {
    ...
    runbook.Automator.clickElement(clickable.element);
