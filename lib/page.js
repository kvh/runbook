...
runbook.Page = class Page {
  ...
  */
  fillAndSubmitForm(action, userData, submitForms, score_threshold=.5) {
    ...
    matches.forEach(match => {
      if (match.value == null) {
        if (match.dtype)
          matches.value = userData.getUserValueForDataType(match);
          // matches.push(userData.getUserValueForDataType(match).then(value => {match.value = value; return match;}));
        else
          matches.value = userData.getUserValueForName(match.name);
    ...
    matches.forEach(m => automator.fillFormInput(m));
    automator.submitForm(form, submitForms);

    // What is useful to caller here?
    // return matches; // Won't necessarily have user data yet
    return {browserOperations:automator.browserOperations, scoring:score, result:form.id};
  }
  ...
  */
  goto(action) {
    ...
    let automator = new runbook.BrowserAutomator();
    automator.addBrowserOperation('click-element', clickable.element);
